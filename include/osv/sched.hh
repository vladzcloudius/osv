...
public:
     ...
     *
     * If there isn't any other runable thread on the current CPU the calling
     * thread will continue to run.
     *
     * The peemtion timer will be set to fire after "preempt_after" time period.
     *
...
struct cpu : private timer_base::client {
     ...
     * based on the runtime of the runable threads on the current CPU.
     *
     * When called from the sched::yield(), ensures that if there are other
     * runable threads on the current CPU (except for the idle thread) there
